	...
	with ImportContext() as ctx:
		...
		class Yolov5(torch.nn.Module):
			def __init__(self, device, nc=3, mode='x'):
				...
				with open(f'./{ctx.expected_path}/data/hyps/hyp.scratch.yaml', 'r') as f:
					hyp = yaml.load(f, Loader=yaml.FullLoader)

				self.model = models.yolo.Model(cfg='{}/{}/yolov5{}.yaml'.format(ctx.expected_path, 'models', self.mode), nc=self.nc, anchors=hyp.get('anchors'))
		...
		if pretrained:
			pretrained_path = '{}/yolov5{}.pt'.format(ctx.expected_path, backbone)
