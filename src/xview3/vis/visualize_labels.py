...
def get_offsets(scene_id):
    if scene_id not in chip_offsets:
        with open(f'./{args.chip_path}/{scene_id}/coords.json', 'r') as f:
...
for i, (scene_id, chip_idx, start_row, start_col) in enumerate(vis_chips):
    # Load background.
    dir = f'./{args.chip_path}/{scene_id}'
    vh_im = numpy.load('{}/vh/{}_vh.npy'.format(dir, chip_idx))
    ...
    skimage.io.imsave('{}/{}_{}_{}_{}.png'.format(args.out_path, i, scene_id, start_row, start_col), chip_im)
